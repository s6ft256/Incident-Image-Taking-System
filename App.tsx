...
export default function App() {
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 main-app-container relative selection:bg-blue-500/30 overflow-x-hidden flex flex-col ${appTheme === 'dark' ? 'bg-[#020617]' : 'bg-white'}`}>
      ...
      {view !== 'auth' && (
        <>
          {/* Consolidated Grouped FAB Controls - RTL Alignment at Bottom Right */}
          <div className="fixed bottom-6 right-6 z-50 flex flex-row-reverse items-center gap-6 pointer-events-none">
            {/* 1. FEED Trigger (Rightmost in row-reverse is logically first item from right) */}
            <div className="flex flex-col items-center gap-2 pointer-events-auto">
              <FeedbackAssistant appTheme={appTheme} userName={userProfile?.name} />
              <span className={`text-[9px] font-black uppercase tracking-[0.2em] ${appTheme === 'dark' ? 'text-slate-400' : 'text-slate-500'}`}>Feed</span>
            </div>

            {/* 2. AI Trigger (Middle) */}
            <div className="flex flex-col items-center gap-2 pointer-events-auto">
              <HSEAssistant appTheme={appTheme} />
              <span className={`text-[9px] font-black uppercase tracking-[0.2em] ${appTheme === 'dark' ? 'text-slate-400' : 'text-slate-500'}`}>AI</span>
            </div>

            {/* 3. POLICIES Trigger (Leftmost) */}
            <div className="flex flex-col items-center gap-2 pointer-events-auto">
              <button 
              ...
              <span className={`text-[9px] font-black uppercase tracking-[0.2em] ${appTheme === 'dark' ? 'text-slate-400' : 'text-slate-500'}`}>Policies</span>
            </div>
          </div>
