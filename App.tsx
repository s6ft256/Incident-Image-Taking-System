...
const SplashScreen: React.FC<{ onComplete: () => void }> = ({ onComplete }) => {
  ...
  return (
    <div className={`fixed inset-0 z-[1000] flex items-center justify-center bg-[#020617] transition-opacity duration-700 ease-in-out ${isVisible ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
      <div className="relative flex flex-col items-center animate-in zoom-in-95 duration-1000">
        {/* Subtle background pulse */}
        <div className="absolute inset-0 bg-blue-500/10 blur-[80px] rounded-full animate-pulse scale-150"></div>
        
        <div className="relative w-48 h-48 sm:w-64 sm:h-64 overflow-hidden rounded-full border-4 border-white/5 shadow-[0_0_50px_rgba(0,0,0,0.5)] bg-white p-2">
          <img 
            src="TROJAN.jpeg" 
            alt="Trojan Construction Group" 
            className="w-full h-full object-contain animate-in fade-in slide-in-from-bottom-4 duration-1000"
          />
        </div>
        
        <div className="mt-10 text-center space-y-2">
          <h2 className="text-white text-2xl font-black tracking-[0.3em] uppercase">HSE Guardian</h2>
      ...
      <div className="absolute bottom-10 left-0 right-0 text-center">
        <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.5em]">TGC GROUP SYSTEMS</p>
      </div>
    </div>
  );
};
