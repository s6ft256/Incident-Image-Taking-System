...
export const CreateReportForm: React.FC<CreateReportFormProps> = ({ baseId, onBack }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      uploadResults.forEach((res, idx) => {
        if (res.status === 'fulfilled') attachments.push({ url: res.value, filename: pendingImages[idx].file.name });
      });
  ...
  if (submitStatus === 'success' || submitStatus === 'offline-saved') {
    return (
      <div className="bg-white dark:bg-white/10 backdrop-blur-2xl border border-slate-200 dark:border-white/20 rounded-3xl shadow-2xl p-10 text-center space-y-6 animate-in zoom-in duration-500">
        ...
        <h2 className="text-3xl font-black text-slate-900 dark:text-white">{submitStatus === 'offline-saved' ? 'Report Logged Offline' : 'Evidence Submitted'}</h2>
        ...
        <div className="flex flex-col gap-3 pt-4">
          ...
          <button onClick={onBack} className="w-full bg-slate-100 dark:bg-white/5 hover:bg-slate-200 dark:hover:bg-white/10 text-slate-700 dark:text-slate-300 font-bold py-4 rounded-2xl transition-all">EXIT TO DASHBOARD</button>
        </div>
      </div>
    );
  }

  const sectionStyles = "bg-white dark:bg-white/5 backdrop-blur-xl rounded-[2rem] border border-slate-200 dark:border-white/10 p-8 shadow-xl dark:shadow-2xl";

  return (
    <div className="animate-in slide-in-from-right duration-500 pb-20">
      <div className="mb-8 relative h-56 rounded-[2.5rem] overflow-hidden shadow-2xl border border-slate-200 dark:border-white/20">
        <img src="https://fatfinger.io/wp-content/uploads/2024/04/foreman-control-loading-containers-box-from-cargo-2024-02-27-21-53-26-utc-min-scaled.jpg" className="w-full h-full object-cover" />
        <div className="absolute inset-0 bg-gradient-to-t from-slate-950 dark:via-slate-950/20 to-transparent"></div>
        <button onClick={onBack} className="absolute top-6 left-6 bg-white/20 dark:bg-white/10 hover:bg-white/30 text-white p-3 rounded-2xl backdrop-blur-xl border border-white/20 transition-all z-10">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className={sectionStyles + " space-y-6"}>
          <h2 className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest border-b border-slate-100 dark:border-white/5 pb-4">Observer Identification</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <InputField id="role" label="Current Role" value={formData.role} onChange={handleInputChange} required options={ROLES} type="select" />
        ...
        {errorMessage && (
          <div className="bg-red-500/10 dark:bg-red-500/20 backdrop-blur-lg border border-red-500/30 text-red-600 dark:text-red-200 p-4 rounded-2xl text-[10px] font-black uppercase tracking-widest text-center">
            {errorMessage}
          </div>
        )}
        ...
        <button
          ...
          className="w-full bg-blue-600 hover:bg-blue-500 disabled:bg-slate-300 dark:disabled:bg-slate-700 text-white font-black py-5 rounded-2xl shadow-xl transition-all active:scale-[0.98] uppercase tracking-widest text-lg"
        >
