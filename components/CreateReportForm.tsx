...
export const CreateReportForm: React.FC<CreateReportFormProps> = ({ baseId, onBack }) => {
  ...
  if (submitStatus === 'success' || submitStatus === 'offline-saved') {
    return (
      <div className="bg-white/10 backdrop-blur-2xl border border-white/20 rounded-3xl shadow-2xl p-10 text-center space-y-6 animate-in zoom-in duration-500">
        ...
        <h2 className="text-3xl font-black text-white">{submitStatus === 'offline-saved' ? 'Report Logged Offline' : 'Evidence Submitted'}</h2>
        ...
        <div className="flex flex-col gap-3 pt-4">
          ...
          <button onClick={onBack} className="w-full bg-white/5 hover:bg-white/10 text-slate-300 font-bold py-4 rounded-2xl transition-all">EXIT TO DASHBOARD</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="animate-in slide-in-from-right duration-500 pb-20">
      <div className="mb-8 relative h-56 rounded-[2.5rem] overflow-hidden shadow-2xl border border-white/20">
