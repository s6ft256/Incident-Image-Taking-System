...
export const CraneChecklistForm: React.FC<CraneChecklistFormProps> = ({ appTheme, onBack }) => {
  ...
  return (
    <div className="animate-in slide-in-from-bottom-5 duration-500 pb-20 max-w-4xl mx-auto relative">
      {/* Thematic Background Image Overlay - Fully clearly seen (0% transparency, 0% blur) */}
      <div className="fixed inset-0 z-0 pointer-events-none">
        <img 
          src={CRANE_BG} 
          className="w-full h-full object-cover" 
          alt="Thematic Crane Background" 
        />
        {/* Subtle vignette to maintain readability without washing out the image */}
      ...
      <div className={`relative z-10 p-6 sm:p-10 rounded-[3rem] border shadow-2xl overflow-hidden backdrop-blur-md ${
        isLight ? 'bg-white/60 border-slate-200/50 shadow-white/10' : 'bg-[#0f172a]/60 border-white/10 shadow-black/20'
      }`}>
          ...
          {CRANE_TYPES.slice(0, 6).map(type => (
            <button 
              ...
              className={`p-3 rounded-2xl border text-[8px] font-black uppercase tracking-widest transition-all ${
                craneType === type ? 'bg-amber-600 border-amber-400 text-white shadow-xl scale-[1.02]' : 'bg-black/40 border-white/10 text-slate-300 hover:bg-black/60'
              }`}
            >
              {type}
            </button>
        ...
        <div className="mt-12 flex flex-col items-center gap-8">
           <div className={`p-6 rounded-3xl border text-center w-full shadow-inner ${isLight ? 'bg-white/40 border-slate-200/50' : 'bg-black/40 border-white/10'}`}>
              <p className={`text-xs font-black uppercase italic leading-relaxed ${isLight ? 'text-slate-600' : 'text-slate-300'}`}>
           ...
           <button 
             ...
             className="w-full sm:w-80 py-6 bg-amber-600 hover:bg-amber-500 text-white font-black rounded-[2rem] uppercase tracking-[0.4em] text-xs shadow-2xl transition-all active:scale-95 border border-amber-400/20"
           >
      ...
      {activeCommentKey && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-300">
           <div className={`w-full max-w-md p-8 rounded-[2.5rem] border shadow-2xl ${isLight ? 'bg-white/95 border-slate-200' : 'bg-[#0f172a]/95 border-white/10'}`}>
              ...
              <div className="flex flex-col gap-3">
                 <button onClick={() => setActiveCommentKey(null)} className="w-full py-4 bg-blue-600 text-white font-black rounded-2xl uppercase text-[10px] tracking-widest hover:bg-blue-500 transition-all">Save</button>
