...
export const RecentReports: React.FC<RecentReportsProps> = ({ baseId, onBack, appTheme = 'dark', filterAssignee }) => {
  ...
  const tabFilteredReports = useMemo(() => {
    return sortedReports.filter(report => {
      ...
      if (isMyTasksMode) {
        return !isClosed && assignedTo === filterAssignee && matchesType;
      }
  ...
  return (
    <div className="animate-in slide-in-from-right duration-300 pb-24">
      ...
      {(activeTab === 'open' || activeTab === 'assigned' || (activeTab === 'closed' && isArchiveUnlocked)) && !isMyTasksMode && (
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <div className="sm:w-64 relative">
              <select 
                ...
                className={`${baseClasses} ${themeClasses} appearance-none pr-10 font-black uppercase text-[10px] tracking-[0.15em]`}
              >
                <option value="All Types">All Incident Types</option>
            ...
            <div className={`text-center py-20 rounded-[2rem] border-2 border-dashed ${isLight ? 'bg-slate-50 border-slate-200 text-slate-400' : 'bg-slate-800/20 border-slate-800 text-slate-500'}`}>
              ...
              {(searchTerm || filterType !== 'All Types') && (
                <button 
                  ...
                  className="mt-4 text-[10px] font-black uppercase text-blue-500 hover:underline tracking-widest"
                >
                  Clear all filters
                </button>
              )}
