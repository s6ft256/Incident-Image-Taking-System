...
interface HSEAssistantProps {
  appTheme?: 'dark' | 'light';
}

export const HSEAssistant: React.FC<HSEAssistantProps> = ({ appTheme = 'dark' }) => {
          ...
          <div className={`flex-1 overflow-y-auto p-4 space-y-4 ${appTheme === 'dark' ? 'bg-slate-900/50' : 'bg-slate-50'} scrollbar-hide`}>
                ...
                <div 
                  className={`max-w-[85%] rounded-2xl px-4 py-2.5 text-sm shadow-md ${
                    msg.role === 'user' 
                      ...
                      : `${appTheme === 'dark' ? 'bg-slate-700 text-slate-200 border-slate-600' : 'bg-white text-slate-800 border-slate-200'} rounded-bl-none border`
                  }`}
                >
            ...
            {isLoading && (
              <div className="flex justify-start">
                 <div className={`${appTheme === 'dark' ? 'bg-slate-700 border-slate-600' : 'bg-white border-slate-200'} rounded-2xl rounded-bl-none px-4 py-3 border flex gap-1.5`}>
